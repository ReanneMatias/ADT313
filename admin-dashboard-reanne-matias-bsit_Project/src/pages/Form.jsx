...
const Form = () => {
  ...
  const handleSave = () => {
    const { title, overview, popularity, releaseDate, voteAverage } = formData;
  
    if (!title || !overview || !popularity || !releaseDate || !voteAverage) {
    ...
    const apiEndpoint = movieId
      ? `/api/edit-movie/${movieId}` // Edit existing movie
      : "/api/save-movie"; // Save new movie
    const apiMethod = movieId ? axios.put : axios.post;
  
    // Call API to save or update the movie
    apiMethod(apiEndpoint, formData)
      .then(() => {
        setIsLoading(false); // Reset loading state
        setError(""); // Clear any errors
        // Navigate to the movie list page after saving
        navigate("/movies"); // This will redirect to your movie list page
      })
      .catch(() => {
        setIsLoading(false);
